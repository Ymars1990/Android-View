...
/**
 * 创建时间:15/6/24 11:21 描述:流式布局
 */
public class BGAFlowLayout extends ViewGroup {
	private List<Row> mRows = new ArrayList<>();
	private int mHorizontalChildGap;// 子控件水平间距
	private int mVerticalChildGap;// 子控件垂直间距
	...
	private void initCustomAttr(int attr, TypedArray typedArray) {
		if (attr == R.styleable.BGAFlowLayout_fl_horizontalChildGap) {
			 ...
			 */
			mHorizontalChildGap = typedArray.getDimensionPixelOffset(attr,
					mHorizontalChildGap);
		} else if (attr == R.styleable.BGAFlowLayout_fl_verticalChildGap) {
			mVerticalChildGap = typedArray.getDimensionPixelOffset(attr,
					mVerticalChildGap);
		} else if (attr == R.styleable.BGAFlowLayout_fl_isDistributionWhiteSpacing) {
			mIsDistributionWhiteSpacing = typedArray.getBoolean(attr,
					mIsDistributionWhiteSpacing);
		}
	}
	...
	public void setOnClick(OnClickListener listener){
		for(int i=0;i<mRows.size();i++){
			mRows.get(i).setOnClickListener(listener);
		}
	}
	private class Row {
